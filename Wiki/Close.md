...
## Workflow You would typically attach `CloseButtonAuthoring` component to the button. This component allows you to store multiple 
...
## Runtime
When the button is clicked, the `ToggleVisibilitySystem` will loop through all the targets that are intended to close. 
...
This enables the `UpdateLocalMeshDataSystem` to manipulate the vertices to either move them out of view or move them back 
into view.
